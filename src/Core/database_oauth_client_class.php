...
class database_oauth_client_class extends oauth_client_class
{
    ...
    Function CreateOAuthSession($user, &$session)
    {
        ...
        $parameters = array(
            ...
            $session->access_token_secret, // string
            $session->expiry, // null
            $session->authorized, // null
            $session->type, // string
            $session->server, // string
            $session->creation, //string
            $session->refresh_token // strin
        );



        if(!$this->Query('INSERT INTO oauth_session
                                     (session, state, access_token,
                                      access_token_secret, expiry,
                                      authorized, type, server,
                                       creation, refresh_token) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', $parameters, $results))
        return false;
    ...
    Function GetUserSession($user, &$oauth_session)
    {
        ...
        if(!$this->Query('SELECT id, session, state, access_token, access_token_secret,
                                expiry, authorized, type, server, creation, refresh_token
                                FROM oauth_session WHERE user=? AND server=? ORDER BY id DESC LIMIT 0,1 ', $parameters, $results ))
            return false;
    ...
    Function GetOAuthSession($session, $server, &$oauth_session)
    {
        ...
        if( !$this->Query('SELECT id, session, state, access_token, access_token_secret, expiry, authorized, type, server, creation, refresh_token
                                FROM oauth_session WHERE session=? AND server=?', $parameters, $results ))
        return false;
